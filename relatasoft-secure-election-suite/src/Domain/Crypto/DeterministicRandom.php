...
class DeterministicRandom {
	...
	public function __construct( string $seed_binary ) {
		if ( strlen( $seed_binary ) < 16 ) {
			throw new CryptoException( 'Deterministic RNG seed too short.' );
		}
	...
	public static function fromHex( string $seed_hex ): self {
		...
		if ( false === $rses_bin ) {
			throw new CryptoException( 'Invalid hex seed.' );
		}
	...
	public function randomIntegerBetween( \GMP $min_inclusive, \GMP $max_inclusive ): \GMP {
		...
		throw new CryptoException( 'Failed to generate deterministic random integer.' );
	}
	...
	public function oddIntegerOfBitLength( int $bits ): \GMP {
		if ( $bits < 2 ) {
			throw new CryptoException( 'Bit length must be at least 2.' );
		}
