...
class ShamirSecretSharing {
	...
	public static function reconstructSecret( array $shares, \GMP $field_prime ): \GMP {
		...
		if ( count( $rses_points ) < 2 ) {
			throw new CryptoException( 'At least 2 shares required for reconstruction.' );
		}
	...
	public static function validateSharePayload( array $payload ): void {
			...
			if ( ! array_key_exists( $rses_key, $payload ) ) {
				throw new CryptoException(
					sprintf(
						...
						'Missing required share field: %s',
						$rses_key
					)
				);
			}
		...
		if ( self::RSES_SHARE_SCHEME !== $payload['scheme'] ) {
			throw new CryptoException( 'Invalid Shamir share scheme.' );
		}

		if ( ! is_array( $payload['public_key'] ) ) {
			throw new CryptoException( 'Invalid public key in share payload.' );
		}

		$rses_pk_required = array( 'p', 'q', 'g', 'y' );
		foreach ( $rses_pk_required as $rses_pk_key ) {
			if ( empty( $payload['public_key'][ $rses_pk_key ] ) ) {
				throw new CryptoException( 'Incomplete public key in share payload.' );
			}
		}
		...
		if ( ! hash_equals( $rses_expected, $rses_checksum ) ) {
			throw new CryptoException( 'Share checksum validation failed.' );
		}
	}
	...
	public static function rses_compute_share_checksum( array $payload ): string {
		...
		$rses_canonical = json_encode( $rses_data, JSON_UNESCAPED_SLASHES );
	...
	private static function rses_validate_split_params( \GMP $secret, int $threshold, int $total_shares, \GMP $field_prime ): void {
		if ( $threshold < 2 ) {
			throw new CryptoException( 'Shamir threshold must be at least 2.' );
		}

		if ( $total_shares < $threshold ) {
			throw new CryptoException( 'Total shares must be >= threshold.' );
		}

		if ( \gmp_cmp( $secret, \gmp_init( 0 ) ) <= 0 ) {
			throw new CryptoException( 'Secret must be positive.' );
		}

		if ( \gmp_cmp( $field_prime, $secret ) <= 0 ) {
			throw new CryptoException( 'Field prime must be greater than secret.' );
		}

		if ( \gmp_cmp( $field_prime, \gmp_init( $total_shares ) ) <= 0 ) {
			throw new CryptoException( 'Field prime must be greater than total shares.' );
		}

		if ( ! PrimeGenerator::isProbablePrime( $field_prime ) ) {
			throw new CryptoException( 'Field prime must be a probable prime.' );
		}
	}
	...
	private static function rses_normalize_share_points( array $shares, \GMP $field_prime ): array {
		...
		foreach ( $shares as $rses_share ) {
			if ( ! isset( $rses_share['x'], $rses_share['y'] ) ) {
				throw new CryptoException( 'Invalid share structure.' );
			}
			...
			if ( \gmp_cmp( $rses_x, \gmp_init( 0 ) ) <= 0 ) {
				throw new CryptoException( 'Share index must be positive.' );
			}

			$rses_x_key = \gmp_strval( $rses_x, 10 );
			if ( isset( $rses_seen_x[ $rses_x_key ] ) ) {
				throw new CryptoException( 'Duplicate share index detected.' );
			}
		...
		if ( count( $shares ) < $threshold ) {
			throw new CryptoException(
				sprintf(
					...
					'Insufficient shares: %1$d provided, %2$d required.',
					count( $shares ),
					$threshold
				)
			);
