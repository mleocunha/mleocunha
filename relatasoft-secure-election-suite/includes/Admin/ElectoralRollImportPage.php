...
class ElectoralRollImportPage {
	...
	public static function rses_render(): void {
		...
		<div class="wrap rses-wrap rses-screen" <?php echo Translator::rses_html_attrs(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
					...
					<p>
						...
						echo esc_html(
							sprintf(
								...
								__( 'Electoral roll import finished. Created: %1$d. Updated: %2$d. Skipped: %3$d.', 'relatasoft-secure-election-suite' ),
								$created,
								$updated,
								$skipped
							)
			...
			<?php if ( ! empty( $_GET['rses_import_errors'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
				<?php
				$raw  = get_transient( 'rses_electoral_roll_errors_' . get_current_user_id() );
				$errs = is_array( $raw ) ? $raw : array();
				delete_transient( 'rses_electoral_roll_errors_' . get_current_user_id() );
				?>
				<?php if ( ! empty( $errs ) ) : ?>
					<div class="rses-panel rses-panel-warning">
						...
						<ul>
							<?php foreach ( array_slice( $errs, 0, 80 ) as $err ) : ?>
			...
			<section class="rses-panel rses-panel-card">
				...
				<p>
					<a class="button" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=rses_download_electoral_roll_sample' ), Nonce::RSES_ACTION_ELECTORAL_ROLL_SAMPLE, '_rses_nonce' ) ); ?>">
	...
	public static function rses_handle_import(): void {
		...
		if ( ! empty( $result['errors'] ) ) {
			set_transient( 'rses_electoral_roll_errors_' . get_current_user_id(), $result['errors'], 10 * MINUTE_IN_SECONDS );
		}
	...
	public static function rses_handle_sample(): void {
		...
		header( 'Content-Disposition: attachment; filename=cadastro-eleitoral-exemplo.csv' );
