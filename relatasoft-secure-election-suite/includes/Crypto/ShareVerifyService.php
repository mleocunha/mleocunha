<?php
/**
 * Offline share verification (Feldman VSS + legacy checksum).
 *
...
class ShareVerifyService {
	...
	public const CODE_LEGACY_CHECKSUM_OK    = 'LEGACY_SHARE_CHECKSUM_OK';
	public const CODE_CHECKSUM_MISMATCH     = 'VSS-CHECKSUM-MISMATCH';
	public const CODE_COMMITMENT_MISMATCH   = 'VSS-COMMITMENT-MISMATCH';
	public const CODE_TRANSCRIPT_MISMATCH   = 'VSS-TRANSCRIPT-MISMATCH';
	 ...
	 *
	 * Routes Feldman vs legacy Shamir by scheme_id / scheme.
	 *
	...
	public static function rses_verify_payload( array $payload ): array {
		...
		if ( ShamirSecretSharing::RSES_SHARE_SCHEME === $scheme || '' === $scheme_id ) {
			return self::rses_verify_legacy_checksum( $payload );
		}

		return self::rses_result(
			false,
			self::CODE_SCHEME_UNSUPPORTED,
			__( 'Unsupported share scheme for verification.', 'relatasoft-secure-election-suite' ),
	 ...
	 * @return array{ok:bool,code:string,message:string,details:array<string,mixed>}
	 */
	private static function rses_verify_legacy_checksum( array $payload ): array {
		if ( empty( $payload['checksum'] ) ) {
			return self::rses_result(
				false,
				self::CODE_MALFORMED,
				__( 'Legacy share is missing a checksum.', 'relatasoft-secure-election-suite' ),
				array()
			);
		}

		$expected = ShamirSecretSharing::rses_compute_share_checksum( $payload );
		if ( ! hash_equals( $expected, (string) $payload['checksum'] ) ) {
			return self::rses_result(
				false,
				self::CODE_CHECKSUM_MISMATCH,
				__( 'Legacy share checksum mismatch.', 'relatasoft-secure-election-suite' ),
				array()
			);
		}
		...
		return self::rses_result(
			true,
			self::CODE_LEGACY_CHECKSUM_OK,
			__( 'Legacy Shamir share checksum is valid. Feldman commitments are not available for this scheme.', 'relatasoft-secure-election-suite' ),
			array(
				'scheme'      => ShamirSecretSharing::RSES_SHARE_SCHEME,
				'share_index' => (int) ( $payload['share_index'] ?? 0 ),
			)
		);
	}
