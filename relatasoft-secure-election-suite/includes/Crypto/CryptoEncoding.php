...
class CryptoEncoding {
	...
	public static function encodeCount( int $m, \GMP $g, \GMP $p ): \GMP {
		if ( $m < 0 ) {
			throw new CryptoException( __( 'Count must be non-negative.', 'relatasoft-secure-election-suite' ) );
		}
	...
	public static function decodeCount( \GMP $encoded, \GMP $g, \GMP $p, int $max_count ): int {
		...
		throw new CryptoException(
			sprintf(
				...
				__( 'Encoded count not found within bounded range [0, %d].', 'relatasoft-secure-election-suite' ),
				$max_count
			)
		);
	}
