...
class PrimeGenerator {
	...
	public static function generateProbablePrime( int $bits ): \GMP {
		if ( $bits < 2 ) {
			throw new CryptoException( __( 'Prime bit length must be at least 2.', 'relatasoft-secure-election-suite' ) );
		}
		...
		throw new CryptoException( __( 'Failed to generate probable prime.', 'relatasoft-secure-election-suite' ) );
	}
	...
	public static function generateSafePrime( int $bits ): array {
		if ( $bits < 3 ) {
			throw new CryptoException( __( 'Safe prime bit length must be at least 3.', 'relatasoft-secure-election-suite' ) );
		}
		...
		throw new CryptoException( __( 'Failed to generate safe prime.', 'relatasoft-secure-election-suite' ) );
	}
	...
	public static function findGeneratorForSafePrime( \GMP $p, \GMP $q ): \GMP {
		...
		throw new CryptoException( __( 'Failed to find subgroup generator.', 'relatasoft-secure-election-suite' ) );
	}
	...
	public static function generatePrimeGreaterThan( \GMP $secret, int $extra_bits = 128 ): \GMP {
		...
		throw new CryptoException( __( 'Failed to generate prime greater than secret.', 'relatasoft-secure-election-suite' ) );
	}
