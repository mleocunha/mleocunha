...
8. **Login / RSV parity** — importer and Votador share `.rsv` cell normalization; hardened parallel login; email≠login collisions refused (`rsv-cadastro-1`, formerly `login-csv-parity-1`).
...
10. **Electoral-roll false max-rows (1.0.27.1)** — double-submit / retried chunks can append the RSV twice (~5000×2 → “mais de 10000 linhas”). Idempotent chunk append, JS busy guard, truncate oversized assembly to declared size (same fix as evolved 1.0.28, version kept under SSS lab lineage).
