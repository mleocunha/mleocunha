...
| Provisionamento | Páginas do sítio **opt-in** (Redirecionamentos); **não** na ativação |
...
- `Ativator` deixa de chamar `rses_provision_pages()` (M5: rotas nativas por defeito)
...
- Textos EN residuales nalguns telas admin (fora do caminho crítico M5)
