...
5. **Adapters** — **Standalone** (caminho operacional): `index.php` / `bin/ve-http`, `NodeRuntime`, identity JSON, painel `/painel`, jornada `/voto`. **WordPress** (Adapter #1, legado de migração): opções, papéis, menus do hospedeiro.
6. **Presentation** — ShellView / CSS em `assets/`; jornada `/voto` no HTTP standalone (e facade legada no Adapter #1).
